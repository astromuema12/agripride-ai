...
export default function TestimonialsPage() {
  ...
  useEffect(() => {
    testimonialService.getApproved(1000, 0).then(({ data }) => {
      setApproved(data);
      setLoading(false);
    });
  }, []);
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white py-16">
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-12 text-center">
          ...
          <p className="mx-auto mt-3 max-w-2xl text-gray-500">{t("testimonials.pageSubtitle")}</p>
        </motion.div>
        ...
        {loading ? (
          ...
          <div className="rounded-lg border border-dashed border-gray-200 p-12 text-center">
            ...
            <p className="text-gray-500">
              {t("testimonials.noTestimonials")}. {t("testimonials.noTestimonialsDesc")}
            </p>
          </div>
        ) : (
                ...
                <Card className="h-full">
                  <CardContent className="p-5">
                    ...
                    <div>
                      ...
                      {(tItem.location || tItem.farm_type) && <div className="text-xs text-gray-500">{[tItem.location, tItem.farm_type].filter(Boolean).join(" · ")}</div>}
                    </div>
                  </CardContent>
                </Card>
              </motion.div>
