...
import { CheckCircle, Loader2, User, Mail } from "lucide-react";
  ...
  useEffect(() => {
    ...
    supportTicketService
      ...
      .catch(() => {
        if (!cancelled) {
          setLoading(false);
          toast.error(t("dashboard.admin.failedToLoadTickets"));
        }
      });
    return () => {
      cancelled = true;
                ...
                <div key={ticket.id} className="rounded-lg border border-gray-100 bg-white p-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-1 flex items-center gap-3 text-xs text-gray-400">
                        <span className="flex items-center gap-1">
                          <User className="h-3 w-3" /> {ticket.user_id?.slice(0, 8)}
                        </span>
                        {ticket.email && (
                          <span className="flex items-center gap-1">
                  ...
                  <div className="mt-3 space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Input placeholder={t("tickets.responsePlaceholder")} value={response[ticket.id] || ""} onChange={(e) => setResponse((prev) => ({ ...prev, [ticket.id]: e.target.value }))} />
